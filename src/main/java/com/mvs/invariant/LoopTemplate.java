...
//        System.out.println(RedlogRunner.rlsimpl(concat));
    ...
    public static void generateInvariant(File file) {
        ...
        if (concat != "") {
            //TextFileModification.modifyCFile(cfilepath, RedlogRunner.rlsimpl(concat));
            System.err.println("Invariant: " + formatInvariant(RedlogRunner.rlsimpl(concat)));
            TextFileModification.modifyCFile(cfilepath, formatInvariant(RedlogRunner.rlsimpl(concat)));
        } else {
            System.err.println("Cannot generate invariants.");
        }
    }
    private static String formatInvariant(String invariants) {
        String[] invList = invariants.split("and");
        return formatInvariant(Arrays.asList(invList));
    }
